...
function ProgressBar() {
  ...
  return (
    <div className="fixed left-0 top-0 z-50 flex w-full flex-col gap-[8px] bg-white px-[20px] pt-[12px]">
      <BackButton />
      ...
      <div className="flex w-full justify-between">
        <div className={pathname === '/club/create' ? 'text-bold12 text-black' : 'text-regular12 text-gray2'}>
          동아리 개설하기
        </div>
        <div className={pathname.split('/')[4] === 'info' ? 'text-bold12 text-black' : 'text-regular12 text-gray2'}>
          동아리 정보 입력
        </div>
        <div className={pathname.split('/')[4] === 'detail' ? 'text-bold12 text-black' : 'text-regular12 text-gray2'}>
          페이지 설정
        </div>
      </div>
    </div>
